...
const RoomDetails = () => {
  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <div className='flex items-senter gap-1 mt-2'>
            <StarRating/>
        ...
        <div className='flex flex-col lg:flex-row mt-6 gap-6'>
            ...
            <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                {room.images.length > 1 && room.images.map((image,index)=>(
                    <img onClick={()=>setMainImage(image)} key={index} src={image} alt='room-image' className={`w-full rounded-xl shadow-md cursor-pointer object-cover ${mainImage===image && 'outline-3 outline-orange-500'}`}></img>
                ))}
            </div>
        </div>
            ...
            <div className='flex flex-col'>
                ...
                <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                    {room.amenities.map((item,index)=>(
                        <div className='flex items-center gap-2 px-3 rounded-lg bg-gray-100 py-2' key={index}>
